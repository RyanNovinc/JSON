...
export interface BuildOpts {
  ...
  /**
   * The multi-phase roadmap derived FRESH from the current profile — injected
   * by assembleMealPlanPromptV2 alongside derivedPhase (same derivePhase call
   * underlies both, so the route section cannot contradict the phase line).
   * Never sourced from roadmapStorage snapshots: those are a historical
   * record for deviation measurement and can be stale on exactly the fields
...
function phaseContextBlock(phase: DerivedPhase, macros: any, planDays: number, hasLeanMassTargets?: boolean, roadmap?: Roadmap | null, completedPhases = 0): string {
  ...
  // The multi-phase journey this plan serves, so the AI stops treating the
  ...
  // phase label above) and when no roadmap could be derived (no goal body
  // fat) — in both cases the block degrades to exactly its pre-route shape.
  if (roadmap && phase !== 'maintain' && roadmap.phases.length > 0) {
    ...
    lines.push('', '**Route (the multi-phase journey this plan serves):**');
    ...
    if (totalPhases === 1) {
      ...
      if (roadmap.gapKg != null && roadmap.gapKg > 0 && roadmap.leanNowKg != null) {
        lines.push(
          `- Lean-mass target: ~${roadmap.leanTargetKg} kg (currently ~${roadmap.leanNowKg} kg — still ~${roadmap.gapKg} kg to build).`,
        );
      }
      lines.push(
        '- Phases end on a body-fat NUMBER, never a date. Body-fat readings carry several points of error, so thresholds apply to the trend.',
        '- The app recomputes calorie and protein targets at each phase change, and the user regenerates their plan then. Do NOT schedule the transition into this plan, do NOT re-derive or reorder the sequence, and NEVER present cutting first as a physiological requirement for building muscle.',
      );
    }
  }
...
export function buildMealPlanPrompt(
  ...
  parts.push(
    [
      ...
      '',
      'Emoji in a HEADING is reserved the same way. The only heading in your entire response allowed to carry an emoji is the summary heading near the end, described under \u201cFinish with Your plan at a glance\u201d. Every other heading is plain text. This exists so the user can find the summary by scrolling without reading.',
    ].join('\n')
  );
  ...
  parts.push(
    [
      ...
      '',
      'Everything above this section is the plan itself \u2014 sixty-odd entries the user did not watch you build. This section is the only part written for someone DECIDING rather than reading, because they are about to be asked whether they are happy with all of it. If they read nothing else, this has to be enough for them to say yes, or to name the one thing they want changed.',
      '',
      'Write it in this exact order, and write nothing else in it:',
      '',
      '1. **One bold line, OUTSIDE the quote:** duration \u00b7 daily calories \u00b7 the protein/carbs/fat split in grams \u00b7 phase.',
      '2. **A slot table, OUTSIDE the quote.** One row per slot the week structure grants (breakfast, lunch, dinner, snacks, dessert). Columns: Slot | Times | What you are eating | Repeats. \u201cTimes\u201d is the occurrence count. \u201cWhat you are eating\u201d names the actual meals, not a category. \u201cRepeats\u201d says how many distinct meals fill that slot, e.g. \u201c1 meal \u00d7 7\u201d or \u201c3 meals rotating\u201d \u2014 this is the single thing users most often want changed, so it must be visible without scrolling the plan.',
      '3. **Everything from here down goes INSIDE a blockquote** \u2014 every line prefixed with a > character, including the blank lines between paragraphs. Three fixed lines first, in this order: what cooking the week actually costs (how many batch sessions, which days, what gets frozen); whether every day landed inside its calorie and protein bands, naming any that did not; and anything invented or filled from a UF row rather than the user\u2019s own picks.',
      '4. **A bold \u201cWhat I decided\u201d label, then AT MOST FIVE bullets, ONE LINE EACH.** These are the JUDGEMENT CALLS you made on their behalf, because those are the only things they might want changed. Do not restate the plan or list its meals back. Cover only where it applies: which of their picked meals you did NOT use and why in a few words (too big for the calorie room, no slot left, allergen); anything that repeats a lot and the reason it had to; any target you could not hit and the constraint that blocked it; anything you had to invent; a slot where their own picks could not fill the week.',
      '5. **One closing sentence** telling them they can change any of it by saying so in plain words, and that they do not need to know how the plan is built to ask for a different breakfast.',
      '',
      'Hard rules for this section:',
      '',
      '- THE BLOCKQUOTE IS NOT DECORATION. It is what makes this section look different from the whole plan above it, so the user can find the part written for them. Every single line inside it starts with a > character. A blank line inside the quote is \u201c> \u201d on its own, NOT an empty line \u2014 an unprefixed blank line ends the quote early and splits the section in two.',
      '- The slot table stays OUTSIDE the quote. Tables nested inside blockquotes render inconsistently across different AI apps, and this prompt is read by several.',
      '- ONE LINE per bullet, not one paragraph per bullet. If a decision needs three sentences to justify, the justification belongs in the plan notes and the bullet carries only the conclusion.',
      '- Five bullets is a ceiling, not a target. Four is better than five.',
      '- No tables other than the slot table. No per-day arithmetic, no meal lists, no grocery figures \u2014 there is no grocery list at this step.',
      '- The whole section must fit on one phone screen. If it does not, cut bullets \u2014 never cut the slot table.',
      ...
      '',
      'Shape to follow, including the > prefixes exactly as shown. The content is illustrative only \u2014 use the real plan:',
      '',
      '## \ud83d\udccb Your plan at a glance',
      '',
      '**7 days \u00b7 2,750 kcal/day \u00b7 195g protein \u00b7 300g carbs \u00b7 92g fat \u00b7 Trim phase**',
      '',
      '| Slot | Times | What you are eating | Repeats |',
      '|---|---|---|---|',
      '| Breakfast | 7 | Baked oats | 1 meal \u00d7 7 |',
      '| Lunch | 7 | Pulled pork bowl, chicken burrito | 2 meals rotating |',
      '| Dinner | 7 | Steak plate, salmon rice, pulled pork sandwich | 3 meals rotating |',
      '| Snacks | 7 | Greek yoghurt, protein bar | 2 meals rotating |',
      '| Dessert | 1 | Chocolate mousse | once, Saturday |',
      '',
      '> **Cooking:** two batch sessions, Sunday and Wednesday. Pulled pork makes 8 serves, the plan eats 6.3, so 2 portions get frozen.',
      '> **Targets:** every day inside its calorie and protein bands.',
      '> **Invented:** nothing \u2014 every meal is one of your picks.',
      '>',
      '> **What I decided**',
      '> - Baked oats runs all 7 days because it is the only breakfast that fits under your morning calorie room',
      '> - Your teriyaki bowl pick went unused, it needs 900 kcal and no slot had that much left',
      '> - Pulled pork appears 6 times to finish the batch rather than waste it',
      '> - Dessert is once, on Saturday, because that is what your week structure grants',
      '',
      '> Don\u2019t like something? Say so in plain words \u2014 a different breakfast, less repetition, fewer cooking days. You don\u2019t need to know how the plan is built to ask, and I\u2019ll tell you what the change costs.',
      '',
...
export async function assembleMealPlanPromptV2(opts?: BuildOpts): Promise<string> {
  ...
  if (profile && profile.goalWeightKg != null) {
    ...
    if (macros) {
      ...
      // Fresh, never from snapshots. Same derivePhase underlies the opener,
      // so this cannot contradict the phase line above it. Null when the
