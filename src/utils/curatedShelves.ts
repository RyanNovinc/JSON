...
export const EQUIPMENT_ORDER: EquipmentType[] = [
  ...
  'rice_cooker', 'oven', 'pressure_cooker', 'grill', 'food_processor',
];
...
// ---- Badge ----
export function mealBadge(meal: CuratedMeal): BadgeKind {
...
export function coreShelfStatus(shelf: CoreShelf, shelfMeals: CuratedMeal[], selected: Set<string>): ShelfStatusResult {
...
export function exoticShelfStatus(shelfMeals: CuratedMeal[], selected: Set<string>): ShelfStatusResult {
  const picked = pickedMealCount(shelfMeals, selected);
...
export function mealTimeBucket(meal: CuratedMeal): TimeBucket {
...
export function mealEquipment(meal: CuratedMeal): Set<EquipmentType> {
...
export function passesFilter(meal: CuratedMeal, f: FilterState): boolean {
...
function nameOf(m: CuratedMeal): string {
  return m.plates?.[0]?.display_name || m.display_name || m.slug;
}
export function mealsForSlots<T extends CuratedMeal>(slots: MealSlot[], all: T[], f: FilterState, sort: SortMode): T[] {
...
export function mealsForCoreShelf<T extends CuratedMeal>(shelf: CoreShelf, all: T[], f: FilterState, sort: SortMode): T[] {
...
export type TrailingCard =
  ...
  | { kind: 'none' };
export function trailingCardState(shelfMeals: CuratedMeal[], selected: Set<string>, expanded: boolean, cap: number = SHELF_CARD_CAP): TrailingCard {
...
export function equipmentChipCounts(all: CuratedMeal[], f: FilterState): EquipmentChip[] {
