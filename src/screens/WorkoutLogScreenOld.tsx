        ...
        >
          ...
          >
            <View style={styles.notesModalContainer}>
              ...
              {/* RIR Scheme Section - only show when reps tab is active and exercise has RIR data */}
              {activeTab === 'reps' && (() => {
                const exercise = dynamicExercises[showNotes.exerciseIndex];
                const hasWeeklyRIR = exercise.rir_weekly && Object.keys(exercise.rir_weekly).length > 0;
                
                if (!hasWeeklyRIR) return null;
                
                return (
                  <View style={styles.repSchemeCard}>
                    <View style={styles.repSchemeTitleContainer}>
                      <Ionicons name="pulse-outline" size={24} color="#ffffff" />
                      <Text style={styles.repSchemeLabel}>RIR Progression</Text>
                    </View>
                    <View style={styles.repSchemeInfo}>
                      <View style={styles.weeklyScheme}>
                        {Object.entries(exercise.rir_weekly).map(([week, rir]) => (
                          <View 
                            ...
                            style={[
                              styles.weekBlock,
                              week === currentWeek.toString() && [styles.weekBlockActive, { backgroundColor: themeColor + '30' }]
                            ]}
                          >
                            <Text style={styles.weekLabel}>Week {week}</Text>
                            <Text style={[
                              styles.weekReps, 
                              { color: week === currentWeek.toString() ? themeColor : '#d4d4d8' }
                            ]}>
                              {rir}
                            </Text>
                          </View>
                      ...
                      <Text style={styles.repSchemeNotes}>
                        RIR (Reps in Reserve): How many reps you could still do before failure. Lower RIR = closer to failure.
                      </Text>
                    </View>
                  </View>
                );
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    paddingHorizontal: 16,
    paddingVertical: 16,
    paddingBottom: 100, // Extra bottom padding to ensure content is scrollable
  },
  exerciseCard: {
