...
5. When finished, provide the download link`;
  ...
  // More robust replacement - find the section and replace it
  const beforeSection = basePrompt.substring(0, basePrompt.indexOf('# CRITICAL: File Output Instructions'));
  const afterSection = basePrompt.substring(basePrompt.indexOf('# JSON Schema Required'));
