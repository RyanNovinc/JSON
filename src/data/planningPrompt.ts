...
- **"What this plan was built from"** — the settings the program was derived from, as a short table: goal, training experience, volume tier, equipment, and phase (with the phase notes). Do NOT put duration, days per week or split in this table. Those appear in the block roadmap and the weekly layout below it, and again in the summary at the end of your response, and a fourth statement of them here makes this section read as the program summary when it is not. This table is a spec sheet: it exists so the reader, and the review step that follows, can see the inputs the plan was built against. The actual summary is the section at the very end.
- **Block roadmap** — one row per block: block number, its week range, its rep focus per exercise category, its emphasis, and its deload week.
