...
const buildPrepSession_1 = require("./buildPrepSession");
// --- tiny assert harness ---------------------------------------------------
let passed = 0;
let failed = 0;
...
function makeMethod(id, total, active, steps, equipment = ['stovetop']) {
    return {
        id,
        display_name: id,
...
function makeMeal(slug, plates, methods, meal_prep) {
    return {
        slug,
        ...
        flex_ingredient_id: 'fixture_flex',
        ...(meal_prep ? { meal_prep } : {}),
    };
}
function planMeal(name, opts = {}) {
    const m = {
    ...
    if (opts.slug !== undefined)
        m.curated_meal_slug = opts.slug;
    if (opts.plateId !== undefined)
        m.plate_id = opts.plateId;
    if ('scale' in opts)
        m.scale_factor = opts.scale; // allow 0 / -2 / NaN through
    return m;
}
function day(date, meals) {
    return { date, dayName: 'X', meals };
}
function makePlan(dailyMeals) {
    return {
        id: 'p',
        name: 'Test',
...
const CURATED = {
    butter_chicken: makeMeal('butter_chicken', [makePlate('standard', { display_name: 'Butter Chicken' })], [makeMethod('stovetop', 45, 30, [step('Cook the curry')], ['stovetop'])], { strategy: 'full', prep_note: 'Cook the curry, store, reheat', storage: { fridge_days: 4, freeze_months: 3 } }),
    beef_stew: makeMeal('beef_stew', [makePlate('standard', { display_name: 'Beef Stew' })], [makeMethod('slow_cooker', 480, 20, [step('Brown beef'), step('Slow cook')], ['slow_cooker'])], { strategy: 'full', prep_note: 'Slow cook, store', storage: { fridge_days: 4, freeze_months: 3 } }),
    chicken_shawarma: makeMeal('chicken_shawarma', [
        makePlate('wrap', {
            display_name: 'Chicken Shawarma Wrap',
            additional: [step('Warm the flatbread'), step('Build with salad + sauce')],
        }),
    ], [makeMethod('oven', 60, 20, [step('Marinate'), step('Roast the chicken')], ['oven'])], { strategy: 'partial', prep_note: 'Cook the chicken; build the wrap fresh', storage: { fridge_days: 4 } }),
    greek_yoghurt_bowl: makeMeal('greek_yoghurt_bowl', [makePlate('standard', { display_name: 'Greek Yoghurt Bowl' })], [makeMethod('no_cook', 5, 5, [step('Combine')], ['no_cook'])], { strategy: 'none', reason: 'Best fresh — yoghurt weeps if pre-mixed' }),
    // meal-level 'full'; the 'tacos' plate OVERRIDES to 'none'; the 'bowl' plate inherits 'full'.
    pulled_pork: makeMeal('pulled_pork', [
    ...
    ], [makeMethod('slow_cooker', 510, 25, [step('Rub'), step('Slow cook 8h'), step('Shred')], ['slow_cooker'])], { strategy: 'full', prep_note: 'Slow cook the pork, store', storage: { fridge_days: 4, freeze_months: 3 } }),
};
// ===========================================================================
...
const a = (0, buildPrepSession_1.buildPrepSession)(planA, CURATED);
...
const b = (0, buildPrepSession_1.buildPrepSession)(planB, CURATED);
...
const c = (0, buildPrepSession_1.buildPrepSession)(planC, CURATED);
...
const d = (0, buildPrepSession_1.buildPrepSession)(makePlan({}), CURATED);
...
const noDaily = (0, buildPrepSession_1.buildPrepSession)({ dailyMeals: undefined }, CURATED);
...
const badDay = (0, buildPrepSession_1.buildPrepSession)(makePlan({ '2026-01-01': { date: '2026-01-01', dayName: 'X', meals: undefined } }), CURATED);
